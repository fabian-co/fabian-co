# Hi, I'm Fabian 👋

📍 Colombia | 🛠️ Full-Stack Engineer | 🤖 AI-Agent Builder

![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![Prisma](https://img.shields.io/badge/Prisma-2D3748?style=for-the-badge&logo=prisma&logoColor=white)
![Gemini](https://img.shields.io/badge/Gemini-8E75C2?style=for-the-badge&logo=googlegemini&logoColor=white)

> After 10 years as a 3D Designer, I became obsessed with programming over the last 3 years, deeply inspired by the evolution of AI. By mastering core fundamentals alongside AI, I aim to build impactful software solutions. Now in vibe-coding mode—shipping AI-powered SaaS and automations at speed.

### 🚀 Projects

* 🏫 **[Kimo Admin](https://kimoadmin.com/)** (In Development) – A comprehensive SaaS for kindergarten management. This is my primary project where I apply everything I learn with the rigor of a real-world, functional product.
    * Leveraging **Next.js**, **NestJS**, and **Prisma** to automate administrative workflows.
    * Integrating **AI into workflows** to optimize lesson planning and administrative tasks.
    * Actively developing new features to provide a complete management ecosystem for educational centers.
* 📈 **[SelfEconomy](https://github.com/fabian-co/SelfEconomy)** – An AI-powered application designed to normalize bank statement data for efficient personal expense tracking.
* 🤖 **AI Automations** – Creating custom agents and workflows using **n8n** and the **Vercel AI SDK**.

### 🏛️ Legacy

* 💸 **[SplitBill](https://github.com/fabian-co/SplitBill)** – One of my first ventures into programming; a utility app focused on clean logic and state management to simplify expense sharing among friends.

---

### 🛠 Tech Stack

* **Backend & DB:** NestJS, Python, Prisma ORM, PostgreSQL.
* **Frontend:** Next.js (App Router), React, Tailwind CSS.
* **Infrastructure:** Railway, Vercel, n8n.

---

### 📈 Currently Learning & Practicing

* 🎓 **Active Learner at Platzi**: Constantly mastering new technologies and following structured learning paths in Full-Stack development.
* Implementing advanced AI features with **Google Gemini** and **Vercel AI SDK**.
* Enhancing English communication for global collaboration.

---

### 📬 Connect with me

[LinkedIn](https://www.linkedin.com/in/fabian-grajales/) 
